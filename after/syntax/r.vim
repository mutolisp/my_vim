...
map <buffer> <F5>	:call RunRProgram("selectedlines")<CR><CR>
imap <buffer> <F5>	<ESC>:call RunRProgram("selectedlines")<CR><CR>a
...
map <buffer> <F3>	:call RunRProgram("long")<CR><CR>
imap <buffer> <F3>	<ESC>:call RunRProgram("long")<CR><CR>a
...
function! RunRProgram(mode) range " {{{
	...
	elseif a:mode == "long"
		...
		let batchFileBuffer = "/tmp/rVim.bat"
		...
		execute '!nohup ' batchFileBuffer ' > /tmp/ppnohup.out &'
